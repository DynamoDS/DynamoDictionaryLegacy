 ...
 function accordionActivate(but, it, obj) {
     ...
     if (obj.Arr != undefined || but.classList.contains("active") == false) {
         ...
         if (it == 1) {
//             console.log(imdiv.selectAll(".Geometry"))
             if(imdiv.selectAll("."+obj.Name)[0].length == 0) {
                 orderedList.forEach(function (d, i) {
                     if (obj.Name == d.Categories[0]) {
                         //         progressBar(i/(orderedList.length-1))
                         var image = getImagePath(d)

                         var tile = imdiv.append("span").append("img").attr("src", image).attr("class",obj.Name).attr("height", 30).attr("width", 30).attr("onerror", "this.onerror=null;this.src='images/src/icon_offset.png';").data([d]).enter()
     ...
     var addAccordion = function (li, appendel, iteration) {
         ...
         li.forEach(function (obj, j) {
             ...
             if (obj["Arr"] != undefined) {
                 ...
                 spanner.append("img").attr("src", image).attr("width", 20).attr("align", "middle").attr("onerror", "this.onerror=null;this.src='images/src/icon_offset.png';")
                 ...
                 if (li.length > 1) {
                     //                     if (obj.Name == ogo.Name) {
                     ////                         console.log(ogo)
                     //                         obj.Name=obj.Name+' '+override(obj);
                     //                         ogo.Name=ogo.Name+' '+override(ogo);
                     ...
                     if (obj.Name == li[(j + 1) % (li.length)].Name) {
                         ...
                         //                         newoo.Name = newoo.Name + ' ' + override(newoo);
                     }
 ...
 function mainPages() {
     var entryText = `Welcome to the Dynamo Dictionary, a searchable database for Dynamo functionality. Here you can find explanations for nodes, sample files, and links to more information on associated workflows.  This site is constantly evolving as the community continues to add more information . . .`;
     imdiv = rightdiv.append("div").append('html').html(entryText).attr("class", "imageTiles").style("margin-left", "3%").style("margin-right", "1%")
 ...
 function updateInfo(ob) {
     ...
     addInputs("Outputs", ".nodeOut", "outs", false)
         //              console.log(JSON.stringify(ob.Categories))
     ...
     if (hit == true) {
         ...
         d3.select(".inDepth").html("<b><br>In Depth:</b>&nbsp&nbsp&nbsp")
         ...
         d3.select(".inDepth").append("html").html("<br>").append("html").html(strr + "<br><br>")
         ...
         d3.select(".exampleFile").html("<hr><br><b>Example File:</b>&nbsp&nbsp&nbsp")
             ...
             //         d3.select(".exampleFile").html("<hr><br><b>In Depth:</b>&nbsp&nbsp&nbsp")
